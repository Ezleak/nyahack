package me.jiyun233.nya.managers
...
class ConfigManager {
    private val path = File(NyaHack.MOD_NAME)
    ...
    private fun initFriend() {
        ...
        if (!friendFile.exists()) {
            ...
            moduleJson.forEach { NyaHack.friendManager!!.add(it.toString()) }
        }
    }
    ...
    private fun initCommand() {
        ...
        if (!commandFile.exists()) {
            ...
            commandJson.addProperty("Prefix", NyaHack.commandPrefix)
            Files.write(
            ...
            NyaHack.commandPrefix = commandJson.get("Prefix").asString
        }
    }
    ...
    private fun clickGuiJson(): JsonObject {
        ...
        for (panel in NyaHack.clickGui!!.panels) {
    ...
    private fun hudEditorJson(): JsonObject {
        val jsonObject = JsonObject()
        for (panel in NyaHack.hudEditor!!.panels) {
            val panelJson = JsonObject()
            panelJson.addProperty("IsShowModules", panel.isShowModules)
            panelJson.addProperty("X", panel.x)
            panelJson.addProperty("Y", panel.y)
    ...
    private fun saveFriends() {
        ...
        Files.write(
            friendFile.toPath(), Gson().toJson(NyaHack.friendManager!!.getAllFriend()).toByteArray(
                StandardCharsets.UTF_8
            )
        )
    }
