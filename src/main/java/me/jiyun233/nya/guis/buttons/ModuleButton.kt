...
import me.jiyun233.nya.guis.buttons.setting.*
...
import me.jiyun233.nya.utils.COG
...
class ModuleButton(width: Float, height: Float, panel: CategoryPanel, val father: AbstractModule) :
    ...
    override fun drawButton(x: Float, y: Float, mouseX: Int, mouseY: Int) {
        if (father.isHud && father.isEnabled) {
            ...
            NyaHack.fontManager.CustomFont.drawString(
                (father.name + " X:" + father.x) + " Y:" + father.y,
                father.x,
                father.y - NyaHack.fontManager.CustomFont.height,
        ...
        NyaHack.fontManager!!.CustomFont.drawStringWithShadow(
            ...
            if (father.isEnabled) ClickGui.getCurrentColor() else Color.WHITE
        )
        NyaHack.fontManager!!.IconFont.drawStringWithShadow(
            COG, (x + width) - 3 - NyaHack.fontManager!!.IconFont.getWidth(
                COG
            ), y + (height / 2) - (NyaHack.fontManager!!.IconFont.height / 4), Color.WHITE
        )
        this.x = x
        this.y = y
    }
