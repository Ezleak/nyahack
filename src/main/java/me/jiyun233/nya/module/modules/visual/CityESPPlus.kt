...
@ModuleInfo(name = "CityESP+", descriptions = "RenderTargetUnsafeHole", category = Category.VISUAL)
object CityESPPlus : Module() {
    ...
    private val burName = registerSetting("BurName", true).booleanVisible(burrow)
    private val highlightCanPlaceCrystal = registerSetting("CanCryHighlight", true)
    ...
    override fun onRender3D(event: Render3DEvent?) {
        mc.world.playerEntities.forEach {
            if (NyaHack.friendManager.isFriend(it) && friend.value) {
                return@forEach
            }
            val pos = BlockPos(it)
            if (it.isBurrowed()) {
            ...
            if (it.isSurround()) {
                surroundPos.forEach { sur ->
                    ...
                    if (BlockUtil.canPlaceCrystal(pos.add(sur.pos2)) && highlightCanPlaceCrystal.value) {
                        color = Color(175, 255, 15, 75)
                    }
